...
def test_valid_event_creation() -> None:
    event = EventEnvelope(
        ...
        payload={"badge_code": "FAST_100"},
    )
    assert isinstance(event.event_id, uuid.UUID)
    assert event.event_type == EventType.BADGE_EARNED
    assert event.event_version == 1
    assert isinstance(event.occurred_at, datetime)
    assert event.occurred_at.tzinfo == timezone.utc
    assert event.producer == "gamification-service"
    assert isinstance(event.correlation_id, uuid.UUID)
    assert event.payload == {"badge_code": "FAST_100"}


def test_auto_event_id_is_uuid4() -> None:
    event1 = EventEnvelope(event_type=EventType.BADGE_EARNED, producer="gamification-service")
    event2 = EventEnvelope(event_type=EventType.BADGE_EARNED, producer="gamification-service")
...
def test_auto_occurred_at_is_utc() -> None:
    event = EventEnvelope(event_type=EventType.BADGE_EARNED, producer="gamification-service")
