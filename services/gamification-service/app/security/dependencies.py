...
def get_current_user(credentials: HTTPAuthorizationCredentials | None = Depends(_bearer)) -> AuthenticatedUser:
    if credentials is None or credentials.scheme.lower() != "bearer":
        raise AppException("TOKEN_INVALID", "Access token is required", 401)
    claims = decode_access_token(credentials.credentials)
    return AuthenticatedUser(uuid.UUID(claims["user_id"]), UserRole(claims["role"]))
