...
def test_valid_event_creation() -> None:
    ...
    assert event.event_type == EventType.TRANSACTION_SCORED
    assert event.event_version == 1
    assert isinstance(event.occurred_at, datetime)
    assert event.occurred_at.tzinfo == timezone.utc
    assert event.producer == "ai-service"
    assert isinstance(event.correlation_id, uuid.UUID)
    assert event.payload == {"risk_score": 0.9}


def test_auto_event_id_is_uuid4() -> None:
    event1 = EventEnvelope(event_type=EventType.TRANSACTION_SCORED, producer="ai-service")
    event2 = EventEnvelope(event_type=EventType.TRANSACTION_SCORED, producer="ai-service")
...
def test_auto_occurred_at_is_utc() -> None:
    event = EventEnvelope(event_type=EventType.TRANSACTION_SCORED, producer="ai-service")
